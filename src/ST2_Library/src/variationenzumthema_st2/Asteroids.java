...
public class Asteroids extends GraphicsProgram implements AsteroidConstants {
	...
	private void checkForCollisionBulletWithAsteroid() {
		if (bullet != null) {
			...
			GObject obj = getElementAt((int) x, (int) y);
	...
	private void checkForCollisionBulletWithWall() {
		if (bullet != null) {
			double x = bullet.getX();
			double y = bullet.getY();
			if ((x < -GBullet.BULLET_SIZE) || (x > getWidth()) || (y < -GBullet.BULLET_SIZE) || (y > getHeight())) {
				remove(bullet);
				bullet = null;
			}
		}
	...
	private void checkForCollisionSpaceShipWithWall() {
		...
		if (x < -GSpaceShip.SPACE_SHIP_SIZE) {
			spaceShip.setLocation(getWidth(), (int) y);
		} else if (x > getWidth()) {
			spaceShip.setLocation(-GSpaceShip.SPACE_SHIP_SIZE, (int) y);
		}
		if (y < -GSpaceShip.SPACE_SHIP_SIZE) {
			spaceShip.setLocation((int) x, getHeight());
		} else if (y > getHeight()) {
			spaceShip.setLocation((int) x, -GSpaceShip.SPACE_SHIP_SIZE);
		}
	}
	...
	private void checkForCollisionAsteroidsWithWall() {
		for (int i = 0; i < NR_OF_ASTEROIDS; i++) {
			...
			if (x < -ASTEROID_SIZE) {
				asteroids[i].setLocation(getWidth(), (int) y);
			} else if (x > getWidth()) {
				asteroids[i].setLocation(-ASTEROID_SIZE, (int) y);
			}
			if (y < -ASTEROID_SIZE) {
				asteroids[i].setLocation((int) x, getHeight());
			} else if (y > getHeight()) {
				asteroids[i].setLocation((int) x, -ASTEROID_SIZE);
			}
		}
	}
	...
	private void createAsteroids() {
		for (int i = 0; i < NR_OF_ASTEROIDS; i++) {
			asteroids[i] = new GAsteroid(rgen.nextInt(getWidth()), rgen.nextInt(getHeight()));
